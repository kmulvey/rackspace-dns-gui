...
DB.prototype.addUser = Step.fn(function(username, key, pass, email, rs_username) {
	...
	mydb.query().insert('users', [ 'name', 'rs_key', 'seq_num', 'last_updt', 'passwd', 'email', 'rs_username' ], [ mysql.escape(username), mysql.escape(key), 1, new Date(), hash, mysql.escape(email), mysql.escape(rs_username) ]).execute(this);
